import { getMax, getPercentFromNumber, Table } from "./1-soc-opt-oop.js";
...
const tests = {
    ...
    normalize: () => {
        ...
        if (result.length !== 2 || result[0][0] !== 'value1' || result[1][1] !== 'value4') {
            throw new Error('Incorrect table structure or content after normalization');
        }
    },
    addPercentFromColumn: () => {
        const testTable = [['City', '100'], ['Town', '50']];
    ...
    sortByColumnIndex: () => {
        const testTable = [['A', '2'], ['B', '1'], ['C', '3']];
        const result = Table.sortByColumnIndex(testTable, 1);
        if (result[0][0] !== 'C' || result[2][0] !== 'B') {
            throw new Error('Table not sorted correctly based on the specified column');
        }
    },
    formatRow: () => {
        ...
        if (result !== 'City        100 50') {
            throw new Error('Incorrect formatting of row based on specified column widths');
        }
    },
    prettify: () => {
        const testTable = [['City', '100', '50'], ['Town', '200', '75']];
        const testWidths = [10, 5, 3];
        const result = Table.prettify(testTable, testWidths);
        if (result[0] !== 'City        100 50' || result[1] !== 'Town        200 75') {
            throw new Error('Incorrect formatting of rows based on specified column widths');
        }
    },
};
